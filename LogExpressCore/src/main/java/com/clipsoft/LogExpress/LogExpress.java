...
import com.clipsoft.LogExpress.util.ModulePathFinder;
...
public class LogExpress {
	...
	private static AtomicReference<ShutdownFuture> ShutdownFutureRef=  new AtomicReference<ShutdownFuture>();
	...
	public final static Configuration cloneConfiguration() {
		try {
			lock.lock();
	...
	public final static void updateConfig(Configuration configure) {
		lock.lock();
		waitForShutdown();
		try {
			if(LoggerContextRef.get() == null) {
	...
	private static final ShutdownFuture createEndShutdownFuture() {
	...
	public final static ShutdownFuture shutdown() {
		lock.lock();
		try {
